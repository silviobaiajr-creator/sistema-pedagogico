...
export const openOccurrenceModal = (incidentToEdit = null) => {
    ...
    if (incidentToEdit) {
        ...
        // BLOQUEIO DE EDIÇÃO (Item 6)
        // Se alguma sub-ação já foi iniciada (convocação agendada), bloqueia a edição dos fatos originais.
        // Isso evita inconsistência com documentos já gerados/assinados.
        const hasStartedActions = incidentToEdit.records.some(r => r.meetingDate || r.contactDate_1 || r.statusIndividual !== 'Pendente');
            ...
            const warningHTML = `
                <div class="mb-4 bg-yellow-50 border-l-4 border-yellow-400 p-4">
                    <div class="flex">
                        ...
                        <div class="ml-3">
                            <p class="text-sm text-yellow-700 font-bold">Edição Restrita</p>
                            <p class="text-xs text-yellow-700 mt-1">Esta ocorrência já possui convocações ou assinaturas em andamento. Para garantir a integridade jurídica, os detalhes do fato (data, descrição, envolvidos) não podem ser alterados.</p>
                        </div>
                    </div>
                </div>`;
            document.querySelector('#occurrence-form .space-y-4').insertAdjacentHTML('afterbegin', warningHTML);

            // Desabilita campos chave
            document.getElementById('occurrence-type').disabled = true;
            occurrenceDateInput.disabled = true;
            document.getElementById('description').disabled = true;
            document.getElementById('student-search-input').disabled = true; // Impede adicionar/remover alunos
            const tags = document.querySelectorAll('.student-tag i'); // Remove botões de delete das tags
            tags.forEach(t => t.style.display = 'none');

            // Providências da Escola ainda podem ser editáveis para ajustes internos? 
            // Geralmente sim, mas o fato gerador não.
        }
        ...
        // Remove alertas antigos se houver (reset visual)
        const oldAlert = document.querySelector('#occurrence-form .bg-yellow-50');
        if (oldAlert) oldAlert.remove();

        // Re-habilita
        document.getElementById('occurrence-type').disabled = false;
        occurrenceDateInput.disabled = false;
        document.getElementById('description').disabled = false;
        document.getElementById('student-search-input').disabled = false;
    }
...
export const initOccurrenceListeners = () => {
    ...
    // Novos Filtros
    document.getElementById('occurrence-filter-year').addEventListener('change', (e) => { state.filtersOccurrences.year = e.target.value; renderOccurrences(); });
    document.getElementById('occurrence-filter-shift').addEventListener('change', (e) => { state.filtersOccurrences.shift = e.target.value; renderOccurrences(); });
    document.getElementById('occurrence-filter-class').addEventListener('change', (e) => { state.filtersOccurrences.class = e.target.value; renderOccurrences(); });

    // Botão Limpar
    document.getElementById('clear-occ-filters-btn').addEventListener('click', () => {
        dom.searchOccurrences.value = '';
        dom.occurrenceStartDate.value = '';
        dom.occurrenceEndDate.value = '';
        document.getElementById('occurrence-filter-type').value = 'all';
        document.getElementById('occurrence-filter-status').value = 'all';
        document.getElementById('occurrence-filter-year').value = 'all';
        document.getElementById('occurrence-filter-shift').value = 'all';
        document.getElementById('occurrence-filter-class').value = 'all';

        state.filterOccurrences = '';
        state.filtersOccurrences = { startDate: null, endDate: null, type: 'all', status: 'all', year: 'all', shift: 'all', class: 'all' };
        renderOccurrences();
    });
